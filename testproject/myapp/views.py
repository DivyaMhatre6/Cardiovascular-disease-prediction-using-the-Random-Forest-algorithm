...
def active(request):
    dff = pd.read_csv("../final_cardio.csv")
...
def chlo(request):
    ...
    #new part
    # smoke1=cholesterol.where(cholesterol.cholesterol==1,axis=0).dropna()
...
def indexPage(request):
    ...
    # new part
    smoke_count=df1.groupby(["smoke"],as_index=False)["cardio"].count()
    print(smoke_count)
    ...
    # Use the trained model directly instead of loading incompatible pickle
    loaded_model = model
...
def predictPrice(request):
    ...
    if(request.method=='POST'):
        ...
        if card == 1:
            ...
            ans = str(prec)+"% chances of Patient being cardio infected"
        if card == 0:
            print("Patient may not be cardio infected")
            ans = str(prec)+"% chances of Patient not being cardio infected"
